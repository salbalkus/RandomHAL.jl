...
function MLJBase.fit(model::HALRegressor, verbosity, X, y, w = nothing)
    params, lasso = fit_hal(X, y, Normal(), model.smoothness, w; standardize = false, nlambda = model.nlambda, nfolds = model.nfolds)
...
function MLJBase.fit(model::HALBinaryClassifier, verbosity, X, y, w = nothing)
    params, lasso = fit_hal(X, [.!(y) y], Binomial(), model.smoothness, w; standardize = false, nlambda = model.nlambda, nfolds = model.nfolds)
    fitresult = (params = params,)
