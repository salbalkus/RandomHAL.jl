...
function MLJBase.fit(model::RandomHALRegressor, verbosity, X, y, w = nothing)
    params, lasso = fit_random_hal(X, y, Normal(), model.smoothness, model.nfeatures, model.sample_balance, w; standardize = false, nlambda = model.nlambda, nfolds = model.nfolds)
...
function MLJBase.fit(model::RandomHALBinaryClassifier, verbosity, X, y::Array{Bool, 1}, w = nothing)
    params, lasso = fit_random_hal(X, [.!(y) y], Binomial(), model.smoothness, model.nfeatures, model.sample_balance, w; standardize = false, nlambda = model.nlambda, nfolds = model.nfolds)
    fitresult = (params = params,)
